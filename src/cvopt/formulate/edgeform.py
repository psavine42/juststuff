...
# ------------------------------------------------------------
class ___ShortestTree(Formulation):
    def __init__(self, args, source, target):
        """
        ...
        self._source = source
        self._target = target
        self.branch = Variable(self.space.num_verts, boolean=True)
    ...
    def as_constraint(self):
        ...
        """
        he_dict = self.space.half_edges.base
        he2e = self.space.half_edges.to_edges_index
        UV = np.zeros((self.space.num_verts, self.space.num_hes), dtype=int)
        VW = np.zeros((self.space.num_verts, self.space.num_hes), dtype=int)
        edge_constraint = np.zeros((self.space.num_edges, self.space.num_hes), dtype=int)

        for edge_ix, (u, v) in he_dict.items():
            UV[u, edge_ix] = 1
            VW[v, edge_ix] = 1
            edge_constraint[he2e[edge_ix], edge_ix] = 1

        path_st = np.zeros(self.space.num_verts, dtype=int)
        path_st[self._target] = 1
        path_st[self._source] = -1

        can_branch = np.ones(self.space.num_verts, dtype=int)
        can_branch[self._target + self._source] = 0

        in_degree = UV @ self.action
        out_degree = VW @ self.action

        C = [
            # same as shortest path, but allow a branch
            in_degree - out_degree + self.branch == path_st,

            # to prevent a random branch just making a cycle
            # only select one half edge per edge
            edge_constraint @ self.action <= 1,

            # indegree can be at moist 1
            in_degree <= 1,

            # branches can not occur at the targets or sources
            self.branch <= can_branch,

            # branch can only happen at vertex V if v has already been activated by edge
            self.branch <= in_degree,
            self.branch <= out_degree
        ]
        return C
    ...
    def display(self):
        ...
        data = Formulation.display(self)
        branches = [i for i, v in enumerate(self.branch.value) if v > 0.5]
        data['vertices'] = self._source + self._target + branches
        return data
